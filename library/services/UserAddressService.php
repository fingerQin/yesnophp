...
class UserAddressService extends BaseService {
    ...
    public static function getSubmitUserAddressDetail($data) {
        if ($data['address_id'] == - 1) { // 如果收货地址是新填写的，则验证有效性。
            ...
            $district_info = $district_model->fetchOne([], [
                    'district_id' => $data['district_id'],'status' => 1 
            ]);
            ...
            $where = [
                    'user_id' => $data['user_id'],'address_id' => $data['address_id'],'status' => 1 
            ];
            ...
            $district_model = new District();
            $district_info = $district_model->fetchOne([], [
                    'district_id' => $address_info['district_id'],'status' => 1 
            ]);
            if (empty($district_info)) {
        ...
        $data = [
                'realname' => $realname,'province_name' => $province_name,'city_name' => $city_name,
                'district_name' => $district_name,'address' => $address,'zipcode' => $zipcode,
    ...
    public static function addAddress($user_id, $realname, $zipcode, $mobilephone, $district_id, $address) {
        $data = [
                'realname' => $realname,'zipcode' => $zipcode,'mobilephone' => $mobilephone,'address' => $address 
        ];
        $rules = [
    ...
    public static function editAddress($user_id, $address_id, $realname, $zipcode, $mobilephone, $district_id, $address) {
        ...
        $user_address_model = new MallUserAddress();
        $address_info = $user_address_model->fetchOne([], [
                'address_id' => $address_id,'status' => 1,'user_id' => $user_id 
        ]);
        if (empty($address_info)) {
        ...
        $ok = $user_address_model->update($data, [
    ...
    public static function deleteAddress($user_id, $address_id) {
        ...
        $data = [
                'status' => 2,'modified_time' => $_SERVER['REQUEST_TIME'] 
        ];
        $where = [
    ...
    public static function getAllAddress($user_id) {
        ...
        $columns = [
                'address_id','realname','zipcode','mobilephone','district_id','address' 
        ];
        $user_address_model = new MallUserAddress();
    ...
    public static function setDefaultAddress($user_id, $address_id) {
        ...
        $user_address_model->update([
                'is_default' => 0 
        ], [
                'user_id' => $user_id,'status' => 1 
        ]);
        $data = [
                'is_default' => 1,'modified_time' => $_SERVER['REQUEST_TIME'] 
        ];
        $where = [
                'address_id' => $address_id,'user_id' => $user_id 
        ];
