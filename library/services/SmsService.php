...
class SmsService extends BaseService {
    ...
    const OP_TYPE_SEND = 1; // 发送。
    const OP_TYPE_CHECK = 2; // 验证。
    ...
    protected static function sendCode($mobilephone, $sms_txt, $code = '') {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, array(
                'mobile' => $mobilephone,'message' => $sms_txt 
        ));
        $res = curl_exec($ch);
        $errno = curl_errno($ch);
        curl_close($ch);
    ...
    protected static function _writeLog($sms_txt, $op_type, $mobilephone, $code, $is_destroy) {
        ...
        $data = [
                'op_type' => $op_type,'mobilephone' => $mobilephone,'sms_txt' => $sms_txt,'sms_code' => $code,
                'is_destroy' => $is_destroy ? 1 : 0,'created_time' => $_SERVER['REQUEST_TIME'] 
        ];
