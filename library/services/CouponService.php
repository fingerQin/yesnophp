...
class CouponService extends BaseService {
    ...
    public static function getUserCouponList($user_id, $is_use = -1, $shop_id = -1, $page = -1, $count = 20) {
        ...
        foreach ($list as $k => $v) {
            $coupon_info = $coupon_model->fetchOne([], [
    ...
    public static function getUserToShopCouponList($shop_id, $page = -1, $count = 20) {
        ...
        $where = ' WHERE status = :status AND shop_id = :shop_id AND get_start_time < :get_start_time ' . ' AND get_end_time > :get_end_time AND expiry_date > :expiry_date ';
    $params = [
            ':status' => 1,':shop_id' => $shop_id,':get_start_time' => $_SERVER['REQUEST_TIME'],
            ':get_end_time' => $_SERVER['REQUEST_TIME'],':expiry_date' => $_SERVER['REQUEST_TIME'] 
    ];
    $order_by = ' ORDER BY coupon_id DESC ';
    $sql = "SELECT COUNT(1) AS count FROM mall_coupon {$where}";
    $default_db = new DbBase();
...
public static function doGetCoupon($user_id, $coupon_id) {
    ...
    $data = [
            'coupon_id' => $coupon_id,'user_id' => $user_id,'shop_id' => $coupon_info['shop_id'],'is_use' => 0,
            'use_time' => 0,'created_time' => $_SERVER['REQUEST_TIME'] 
    ];
...
public static function addCoupon($user_id, $shop_id, $get_start_time, $get_end_time, $limit_quantity, $coupon_name, $money, $order_money, $expiry_date) {
    $data = [
            'user_id' => $user_id,'shop_id' => $shop_id,'coupon_name' => $coupon_name,'money' => $money,
            'order_money' => $order_money,'expiry_date' => $expiry_date,'get_start_time' => $get_start_time,
            'get_end_time' => $get_end_time,'limit_quantity' => $limit_quantity 
    ];
    $rules = [
...
public static function editCoupon($user_id, $shop_id, $coupon_id, $get_start_time, $get_end_time, $limit_quantity = 1, $coupon_name, $money, $order_money, $expiry_date) {
    $data = [
            'coupon_id' => $coupon_id,'user_id' => $user_id,'shop_id' => $shop_id,'coupon_name' => $coupon_name,
            'money' => $money,'order_money' => $order_money,'expiry_date' => $expiry_date,
            'get_start_time' => $get_start_time,'get_end_time' => $get_end_time,'limit_quantity' => $limit_quantity 
    ];
    $rules = [
    ...
    $where = [
            'coupon_id' => $coupon_id,'shop_id' => $shop_id,'status' => 1 
    ];
    $coupon_model = new MallCoupon();
    ...
    $user_coupon_model = new MallUserCoupon();
    $user_coupon_info = $user_coupon_model->fetchOne([], [
            'coupon_id' => $coupon_id 
    ], 'id DESC');
    if (! empty($user_coupon_info)) {
    ...
    $coupon_id = $coupon_model->update($data, [
            'coupon_id' => $coupon_id 
    ]);
    if ($coupon_id == 0) {
...
public static function getCouponDetail($shop_id, $coupon_id) {
    $where = [
            'shop_id' => $shop_id,'coupon_id' => $coupon_id,'status' => 1 
    ];
    $coupon_model = new MallCoupon();
    $coupon_info = $coupon_model->fetchOne([], $where);
    if (empty($coupon_info)) {
