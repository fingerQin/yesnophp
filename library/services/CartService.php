...
class CartService extends BaseService {
     ...
     * @return array
     */
    public static function getUserCartList($user_id, $shop_id = -1) {
        $where = [
    ...
    protected static function formatGoods($goods_list) {
        ...
        $group_by_goods_list = []; // 按照店铺分组购物车商品。
        foreach ($goods_list as $key => $goods) {
            ...
            $group_by_goods_list[$goods_info['shop_id']]['goods_list'] = $goods;
        }
        // 取商家信息。
        foreach ($group_by_goods_list as $shop_id => $goods) {
            $shop_model = new MallShop();
            $shop_info = $shop_model->fetchOne([], [
                    'shop_id' => $shop_id 
            ]);
            $group_by_goods_list[$shop_id]['shop_name'] = $shop_info['shop_name'];
        }
        unset($goods_list, $goods_info, $product_info, $goods_model, $product_model);
        return $group_by_goods_list;
    }
    ...
    public static function setUserCartGoodsQuantity($user_id, $goods_id, $product_id, $quantity) {
        ...
        $where = [
                'user_id' => $user_id,'product_id' => $product_id,'status' => 1 
        ];
        $cart_model = new MallCart();
        $cart_info = $cart_model->fetchOne([], $where);
        if (empty($cart_info)) {
            $data = [
                    'user_id' => $user_id,'shop_id' => $goods_info['shop_id'],'goods_id' => $goods_id,
                    'product_id' => $product_id,'quantity' => $quantity,'status' => 1,
            ...
            $data = [
                    'quantity' => $quantity,'modified_time' => $_SERVER['REQUEST_TIME'] 
            ];
            $where = [
                    'user_id' => $user_id,'product_id' => $product_id,'status' => 1 
            ];
    ...
    public static function deleteUserCartGoods($user_id, $goods_id, $product_id) {
        $where = [
                'user_id' => $user_id,'product_id' => $product_id,'goods_id' => $goods_id,'status' => 1 
        ];
        $updata = [
                'modified_time' => $_SERVER['REQUEST_TIME'],'status' => 2 
        ];
        $cart_model = new MallCart();
