...
class FreightService extends BaseService {
     ...
     * @return array
     */
    public static function calculateGoodsFreight($shop_id, $goods_list, $user_district_id = -1) {
        if (empty($goods_list)) {
        ...
        if ($district_id != - 1) {
            ...
            $district_info = $district_model->fetchOne([], [
                    'district_id' => $user_district_id,'status' => 1 
            ]);
            if (empty($district_info)) {
                YCore::exception(- 1, '区县ID有误');
            }
            $city_info = $district_model->fetchOne([], [
                    'city_code' => $district_info['city_code'],'region_type' => 2,'status' => 1 
            ]);
            if (empty($city_info)) {
                YCore::exception(- 1, '配送城市数据有误');
            }
            $province_info = $district_model->fetchOne([], [
                    'province_code' => $district_info['province_code'],'region_type' => 1,'status' => 1 
            ]);
        ...
        foreach ($goods_list as $goods) {
            $result = self::calculateSingleGoodsFreight($shop_id, $goods['goods_id'], $goods['product_id'], $goods['quantity'], $province_id, $city_id, $district_id);
        ...
        return [
                'total_freight' => $total_freight,'goods_list_freight' => $goods_list_freight 
        ];
    }
     ...
     * @return array
     */
    protected static function calculateSingleGoodsFreight($shop_id, $goods_id, $product_id, $quantity, $province_id = 0, $city_id = 0, $district_id = 0) {
        $goods_model = new MallGoods();
        $where = [
                'goods_id' => $goods_id,'shop_id' => $shop_id,'status' => 1 
        ];
        ...
        $product_info = $product_model->fetchOne([], [
                'goods_id' => $goods_id,'product_id' => $product_id,'status' => 1 
        ]);
        ...
        $freight_tpl_info = $freight_tpl_model->fetchOne([], [
                'shop_id' => $shop_id,'tpl_id' => $goods_info['freight_tpl_id'],'status' => 1 
        ]);
        ...
        if (! $is_delivery) {
            return [
                    'is_delivery' => 0,'fregith' => 0 
            ];
        }
        // [2] 判断是否买家包邮。
        if ($freight_tpl_info['bear_freight'] == 1) {
            return [
                    'is_delivery' => 1,'fregith' => 0 
            ];
        }
        // [3] 计件 & 计重 运费计算。
     ...
     * @return array
     */
    public static function getShopFreightList($shop_id) {
        $where = [
        ...
        foreach ($list as $k => $v) {
            $v['bear_freight'] = ($v['bear_freight'] == 1) ? '卖家承担' : '买家承担';
            $v['fright_type'] = ($v['fright_type'] == 1) ? '计件' : '计重';
    ...
    public static function addFreightTpl($data) {
        ...
        $rules = [
                ...
                'shop_id' => '商家ID|require:1000000','freight_name' => '运费模板名称|require:1000000|len:1000000:1:20:1',
    ...
    public static function editFreightTpl($data) {
        ...
        $freight_tpl_model = new MallFreightTpl();
        $where = [
                'tpl_id' => $data['tpl_id'],'shop_id' => $data['shop_id'],'status' => 1 
        ];
        ...
        unset($data['user_id'], $data['shop_id'], $data['tpl_id']);
     ...
     * @return boolean
     */
    public static function deleteFreightTpl($user_id, $shop_id, $tpl_id) {
        $freight_tpl_model = new MallFreightTpl();
        $where = [
                'tpl_id' => $tpl_id,'shop_id' => $shop_id,'status' => 1 
        ];
        $freight_tpl_info = $freight_tpl_model->fetchOne([], $where);
        if (empty($freight_tpl_info)) {
            YCore::exception(- 1, '运费模板不存在或已经删除');
     ...
     * @return array
     */
    public static function getFreightTplDetail($tpl_id, $shop_id) {
        $freight_tpl_model = new MallFreightTpl();
        $where = [
                'tpl_id' => $tpl_id,'shop_id' => $shop_id,'status' => 1 
        ];
