...
class FreightService extends BaseService {
	...
	protected static function calculateSingleGoodsFreight($shop_id, $goods_id, $product_id, $quantity, $province_id = 0, $city_id = 0, $district_id = 0) {
		...
		$product_info = $product_model->fetchOne([], ['goods_id' => $goods_id, 'product_id' => $product_id, 'status' => 1]);
		if (empty($product_info)) {
			YCore::exception(-1, '货品数据有误');
		}
		$freight_tpl_model = new MallFreightTpl();
		$freight_tpl_info = $freight_tpl_model->fetchOne([], ['shop_id' => $shop_id, 'tpl_id' => $goods_info['freight_tpl_id'], 'status' => 1]);
