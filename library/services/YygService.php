...
class YygService extends BaseService {
    ...
    public static function changeYygStartStatus($admin_id, $yyg_id, $start_status) {
        ...
        if ($start_status == 1) {
            ...
            $where = [
                    'qh_number' => $yyg_detail['qh_number'],'yyg_id' => $yyg_id 
            ];
            ...
            if ($qh_detail['winner_id'] != 0) { // 说明已经结束了。
                ...
                $data = [
                        'yyg_id' => $yyg_id,'created_time' => $_SERVER['REQUEST_TIME'],'qh_number' => $qh_number 
                ];
            ...
            $updata = [
                    'modified_time' => $_SERVER['REQUEST_TIME'],'modified_by' => $admin_id,'yyg_start' => 1 
            ];
            $ok = $yyg_model->update($updata, $where);
            if (! $ok) {
                $default_db->rollBack();
            ...
            $updata = [
                    'modified_time' => $_SERVER['REQUEST_TIME'],'modified_by' => $admin_id,'yyg_start' => 0 
            ];
            $ok = $yyg_model->update($updata, $where);
            if (! $ok) {
                $default_db->rollBack();
    ...
    public static function joinYyg($user_id, $yyg_id, $qh_number, $do_times) {
        ...
        $data = [
                'yyg_id' => $yyg_id,'qh_number' => $qh_number,'user_id' => $user_id,'do_times' => $do_times,
    ...
    public static function getYygDetail($yyg_id) {
        ...
        $columns = [
                'yyg_id','qh_id','yyg_name','yyg_image_url','yyg_desc','yyg_price','yyg_richtext','yyg_start' 
        ];
        $yyg_model = new GmYyg();
    ...
    protected static function setYygImage($admin_id, $yyg_id, array $album) {
        ...
        foreach ($album as $image_url) {
            if (! empty($old_image) && in_array($image_url, $old_image)) { // 存在。
                ...
                $insert_data = [
                        'yyg_id' => $yyg_id,'image_url' => $image_url,'status' => 1,
                        'created_time' => $_SERVER['REQUEST_TIME'],'created_by' => $admin_id 
                ];
        ...
        if (! empty($not_exist_image_id)) {
            ...
            $sql = "UPDATE gm_yyg_image SET status = :status, modified_by = :modified_by, modified_time = :modified_time " . "WHERE image_id IN ({$result['question']})";
