...
class NewsService extends BaseService {
    ...
    public static function getNewsDetail($news_id, $is_all = false) {
        ...
        if ($is_all) {
            ...
            $news_data = $news_data_model->fetchOne([], [
                    'news_id' => $news_id 
            ]);
            if ($news_data) {
    ...
    public static function addNews($admin_id, $code, $cat_id, $title, $intro, $keywords, $source, $image_url, $content, $display = 1) {
        ...
        $news_model = new News();
        $news_detail = $news_model->fetchOne([], [
                'code' => $code,'status' => 1 
        ]);
        ...
        $data = [
                'title' => $title,'code' => $code,'intro' => $intro,'keywords' => $keywords,'source' => $source,
                'image_url' => $image_url,'content' => $content,'display' => $display 
        ];
        $rules = [
                ...
                'source' => '文章来源|require:1000000|len:1000000:1:50:1','image_url' => '文章图片|len:1000000:1:100:1',
    ...
    public static function editNews($admin_id, $news_id, $code, $cat_id, $title, $intro, $keywords, $source, $image_url, $content, $display = 1) {
        ...
        $news_detail_code = $news_model->fetchOne([], [
                'code' => $code,'status' => 1 
        ]);
    ...
    public static function sortNews($admin_id, $listorders) {
        ...
        foreach ($listorders as $news_id => $sort_value) {
            $data = [
                    'listorder' => $sort_value,'modified_by' => $admin_id,'modified_time' => $_SERVER['REQUEST_TIME'] 
            ];
            $where = [
                    'news_id' => $news_id,'status' => 1 
            ];
