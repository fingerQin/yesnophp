...
class DistrictService extends BaseService {
            ...
            case 1 :
                ...
                foreach ($list as $item) {
                    if ($item['region_type'] == $region_type) {
                        $result[] = [
                                'id' => $item['district_id'],'name' => $item['province_name'],
            ...
            case 2 :
                ...
                foreach ($list as $item) {
                    if ($item['region_type'] == $region_type && $code == $item['province_code']) {
                        $result[] = [
                                'id' => $item['district_id'],'name' => $item['city_name'],
            ...
            case 3 :
                ...
                foreach ($list as $item) {
                    if ($item['region_type'] == $region_type && $code == $item['city_code']) {
                        $result[] = [
                                'id' => $item['district_id'],'name' => $item['district_name'],'sub' => [] 
                        ];
                    }
                }
                return $result;
    ...
    public static function getByDistrictOfName($region_type, $name) {
        ...
        switch ($region_type) {
            case 1 :
                $where = [
                        'province_name' => $name,'region_type' => 1 
                ];
            ...
            case 3 :
                $where = [
                        'district_name' => $name,'region_type' => 3 
                ];
                $district_info = $district_model->fetchOne([], $where);
                return $district_info ? $district_info['district_id'] : 0;
                break;
            case 4 :
                $where = [
                        'street_name' => $name,'region_type' => 4 
                ];
                $district_info = $district_model->fetchOne([], $where);
                return $district_info ? $district_info['district_id'] : 0;
                break;
