...
class GoodsService extends BaseService {
	...
	public static function getShopGoodsList($shop_id, $updown = -1, $goods_name = '', $cat_id = -1, $custom_cat_id = -1, $start_price = '', $end_price = '', $page = 1, $count = 20) {
		$offset     = self::getPaginationOffset($page, $count);
		$from_table = ' FROM mall_goods ';
		$columns    = ' goods_id,goods_name,slogan,min_market_price,max_market_price,min_price,'
				    . ' max_price,goods_img,buy_count,month_buy_count,marketable,marketable_time,limit_count ';
    ...
    public static function addGoods($data) {
        ...
        if ($data['custom_cat_id'] != -1) {
        	...
        	$shop_cat_info = $shop_category_model->fetchOne([], ['cat_id' => $data['custom_cat_id']]);
        ...
        $insert_data = [
        	...
        	'custom_cat_id'    => $data['custom_cat_id'],
    		...
    		case 2: // 只对差异化部分进行操作。
    			...
    			if (!empty($diff_result['cut_spec_val'])) { // 处理规格值减少的货品。将这部分货品删除。
    				foreach ($diff_result['cut_spec_val'] as $cut_spec) {
    					...
    					$sql = 'UPDATE mall_product SET status = :status, modified_by = :modified_by, modified_time = :modified_time '
    						 . 'WHERE goods_id = :goods_id AND spec_val LIKE :spec_val AND status = :status2';
    					$params = [
    						':status'        => 2,
    						':status2'       => 1,
    						':modified_by'   => $user_id,
    ...
    protected static function diffOldAndNewSpec($old_spec, $new_spec) {
        ...
        return [
            'add_spec_val' => $add_spec_val,
            'cut_spec_val' => $cut_spec_val,
            'diff_type'    => $diff_type
        ];
    }
    ...
    protected static function setGoodsImage($user_id, $goods_id, $album) {
		...
		if (!empty($not_exist_image_id)) {
			...
			$sql = "UPDATE mall_goods_image SET status = :status, modified_by = :modified_by, modified_time = :modified_time "
