...
class AppraiseService extends BaseService {
	...
	public static function statsShopAppraise($shop_id) {
		$default_db = new DbBase();
		$sql = 'SELECT COUNT(1) AS count, SUM(score1) AS score1, SUM(score2) AS score2, SUM(score3) AS score3 '
			 . 'FROM mall_appraise_detail WHERE shop_id = :shop_id';
		$params = [
			':shop_id' => $shop_id
		];
		...
		$appraise_info = $appraise_model->fetchOne([], ['shop_id' => $shop_id]);
		$data = [
			't1' => $result['count'],
			's1' => $result['score1'],
			'p1' => round(($result['score1']/$result['count']), 1),
			't2' => $result['count'],
			's2' => $result['score2'],
			'p2' => round(($result['score2']/$result['count']), 1),
			't3' => $result['count'],
			's3' => $result['score3'],
			'p3' => round(($result['score3']/$result['count']), 1),
		];
		if (empty($appraise_info)) {
			...
			$ok = $appraise_model->update($data, ['aid' => $appraise_info['aid']]);
			if (!$ok) {
				YCore::exception(-1, '服务器繁忙,请稍候重试');
			}
		}
	...
	public static function sellerAppraiseReply($user_id, $shop_id, $sub_order_id, $reply_type, $comment) {
		...
		$order_model = new MallOrder();
		$order_info = $order_model->fetchOne([], ['order_id' => $order_item_info['order_id'], 'shop_id' => $shop_id]);
		if (empty($order_info)) {
			YCore::exception(-1, '评价异常,请稍候刷新重试');
		}
		$comment_model = new MallComment();
