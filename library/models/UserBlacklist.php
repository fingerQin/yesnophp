...
class UserBlacklist extends DbBase {
    ...
    public function isForbidden($user_id) {
        ...
        if ($result['ban_type'] == 2 && ($result['ban_end_time'] < $current_timestamp ||
             $result['ban_start_time'] >
             $current_timestamp)) {
    ...
    public function forbiddenUser($admin_id, $user_id, $username, $ban_type, $ban_start_time = 0, $ban_end_time = 0, $ban_reason = '') {
        ...
        $data = [
                'user_id' => $user_id,'username' => $username,'ban_type' => $ban_type,
                'ban_start_time' => $ban_start_time,'ban_end_time' => $ban_end_time,'ban_reason' => $ban_reason,
                'created_by' => $admin_id,'created_time' => $_SERVER['REQUEST_TIME'],'status' => 1 
        ];
    ...
    public function unforbiddenUser($user_id, $admin_id) {
        ...
        if ($data) {
            $update_data = [
                    'status' => 0,'modified_by' => $admin_id,'modified_time' => $_SERVER['REQUEST_TIME'] 
            ];
