...
use common\YCore;
class UsersBlacklist extends DbBase {

	/**
	 * 表名。
	 * @var string
	 */
	protected $_table_name = 'ms_users_blacklist';
	...
	public function forbiddenUser($admin_id, $user_id, $username, $ban_type, $ban_start_time = 0, $ban_end_time = 0, $ban_reason = '') {
		if ($ban_type == 1) { // 永久封禁不需要设置时间。
			...
			if ($ban_start_time == 0 || strlen($ban_start_time) != 10) {
				YCore::throw_exception(6001000, 'The ban_start_time parameters is wrong');
			}
			if ($ban_end_time == 0 || strlen($ban_end_time) != 10) {
				YCore::throw_exception(6001001, 'The ban_end_time parameters is wrong');
			}
		}
		$data = [
			'user_id'        => $user_id,
	...
	public function unforbiddenUser($user_id) {
		...
		return $this->update(['status' => 0], ['id' => $data['id']]);
	}
}