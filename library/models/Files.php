...
class Files extends DbBase {
    ...
    public function getFile($arr_file_id) {
        ...
        $sql = "SELECT file_id,file_name FROM {$this->_table_name} " . "WHERE file_id IN({$where_in_params['question']}) AND status = :status";
    $params = [
            ':status' => 1 
    ];
...
public function addFiles($file_name, $file_type, $file_size, $file_md5, $user_type = 2, $user_id = 0) {
    $data = [
            'file_name' => $file_name,'file_type' => $file_type,'file_size' => $file_size,'file_md5' => $file_md5,
            'user_type' => $user_type,'user_id' => $user_id,'status' => 1,'created_time' => $_SERVER['REQUEST_TIME'] 
    ];
