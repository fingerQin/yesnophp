...
<div class="subnav">
    <div class="content-menu ib-a blue line-x">
    	<a class="add fb" href="javascript:postDialog('addAccount', '<?php echo YUrl::createBackendUrl('', 'WeChat', 'addAccount'); ?>', '添加公众号', 550, 460)"><em>添加公众号</em></a>
...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		...
		<div class="explain-col">
     		<p>
     		公众号微信号：<input type="text" value="<?php echo $account; ?>" class="input-text" name="account" placeholder="请输入要查询的公众号微信号" />
     		APPID：<input type="text" value="<?php echo $appid; ?>" class="input-text" name="appid" placeholder="请输入要查询的公众号APPID" />
     		公众号编码：<input type="text" value="<?php echo $sn; ?>" class="input-text" name="sn" placeholder="请输入要查询的公众号编码" />
...
<table width="100%" cellspacing="0">
    ...
    <?php foreach ($list as $item): ?>
    	<tr>
    		...
    		<td align="center">
    		  [<a href="###" onclick="edit(<?php echo $item['account_id'] ?>, '<?php echo $item['wx_account'] ?>')" title="修改">修改</a>]&nbsp;
    		  [<a href="###" onclick="deleteDialog('deleteAccount', '<?php echo YUrl::createBackendUrl('', 'WeChat', 'deleteAccount', ['account_id' => $item['account_id']]); ?>', '<?php echo $item['wx_account'] ?>')" title="删除">删除</a>]<br />
    		  [<a href="###" onclick="newsManage(<?php echo $item['account_id'] ?>, '<?php echo $item['wx_account'] ?>')" title="图文管理">图文管理</a>]&nbsp;
    		  [<a href="###" onclick="menuManage(<?php echo $item['account_id'] ?>, '<?php echo $item['wx_account'] ?>')" title="菜单管理">菜单管理</a>]
    		</td>
    	</tr>
    <?php endforeach; ?>
    </tbody>
