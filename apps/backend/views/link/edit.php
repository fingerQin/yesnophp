...
<table width="100%" class="table_form contentWrap">
      <tr>
        ...
        <option <?php echo $menu['cat_id'] == $detail['cat_id'] ? 'selected="selected"' : ''; ?> value="<?php echo $menu['cat_id']; ?>"><?php echo $menu['cat_name']; ?></option>
			<?php if (isset($menu['sub'])): ?>
			<?php foreach ($menu['sub'] as $sub_m): ?>
			<option <?php echo $menu['cat_id'] == $detail['cat_id'] ? 'selected="selected"' : ''; ?> value="<?php echo $sub_m['cat_id']; ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├─<?php echo $sub_m['cat_name']; ?></option>
			<?php if (isset($sub_m['sub'])): ?>
			<?php foreach ($sub_m['sub'] as $ss_m): ?>
			<option <?php echo $menu['cat_id'] == $detail['cat_id'] ? 'selected="selected"' : ''; ?> value="<?php echo $ss_m['cat_id']; ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;│&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├─<?php echo $ss_m['cat_name']; ?></option>
			<?php endforeach; ?>
			<?php endif; ?>
			<?php endforeach; ?>
			<?php endif; ?>
    ...
    <tr>
        <th> 友情链接名称：</th>
        <td><input type="text" name="link_name" id="link_name" style="width:120px;" value="<?php echo htmlspecialchars($detail['link_name']); ?>" class="input-text" ></td>
    </tr>
	<tr>
        <th>友情链接URL：</th>
        <td><input type="text" name="link_url" id="link_url" style="width:250px;" class="input-text" value="<?php echo htmlspecialchars($detail['link_url']); ?>" /></td>
    </tr>
	<tr>
        <th>是否显示：</th>
        <td>
            <select name="display">
            <option <?php echo $menu['display'] ? 'selected="selected"' : ''; ?> value="1">是</option>
            <option <?php echo !$menu['display'] ? 'selected="selected"' : ''; ?> value="0">否</option>
            </select>
        </td>
    </tr>
    <tr>
		...
		<td>
            ...
            <div id="previewImage"><img style="max-width:119px;max-height:119px;" src="<?php echo $detail['image_url'] ? YUrl::filePath($detail['image_url']) : ''; ?>" /></div>
		</td>
	</tr>
    <tr>
